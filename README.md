...
- Pending local hash: `local-8e3fd21c`
- Git code commit hash: pending until upload finalization
- Current version result folder: `results/local-8e3fd21c/`
...
- Hash finalization metadata: this branch will replace `local-8e3fd21c` with the Git code commit hash after upload. The earlier `local-2f83ff50` smoke record is left as a local-hash trace entry.
...
- `docs/comments.md` 最新评阅针对 `a5a22d2` V3-Revised；当前 pending 批次是 `local-8e3fd21c`，因此不能把旧 comments 直接当作当前版本评价。
- 当前 README 只把 comments 用作背景；当前版本的直接证据来自 `docs/research-log.md`、`algorithms/proposed_algorithm_v3_3.md` 和 `results/local-8e3fd21c/`。
- `docs/comments.md` 没有包含 `local-8e3fd21c`，本次 hash finalization 不应改写 comments。
- 如果后续要评阅当前 V3.3 common-snapshot 版本，应以本次上传产生的 Git code commit hash 为 review target。

## 仍需保留的边界

- `local-8e3fd21c` 是文档批次，不是实验 rerun；`602158e` 是 Case 9 screening rerun，不是完整 paper-profile full run。
