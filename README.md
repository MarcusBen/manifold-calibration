...
截至 2026-04-20，当前工作分支是 `codex/proposed_v3`，最新当前批次是 `results/a5a22d2/`。这一批实现并筛选 **Proposed V3-Revised = Calibration-Guarded ARD-Anchored Task-Aware Residual Calibration**，只运行 Case 3 / 7 / 9 / 10，用于判断是否值得进入完整 `paper` profile。
...
- `docs/comments.md` 的最新评阅绑定 Git code commit `a5a22d2`，与当前 V3-Revised screening 匹配。
- 算法说明文件已从根目录收纳到 `algorithms/`，这是有意的文档整理，不是误删。

## Version Trace

- Git code commit hash: `a5a22d2`
- Current version result folder: `results/a5a22d2/`
- Base HEAD for current V3-Revised screening: `7a31dd1`
- Working branch: `codex/proposed_v3`
- Latest reviewed comments hash: `a5a22d2`
- Review status: comments match the current V3-Revised code/results commit.
- Hash finalization metadata: this branch also contains the follow-up metadata commit that replaces the pending trace token with `a5a22d2`.
...
- `a5a22d2` 是 screening run，不是完整 paper-profile full run。
