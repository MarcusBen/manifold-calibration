...
### 2026-05-13: `4cfa7eb` parallel backend family plumbing validation
...
- Validation:
  ...
  - Traceable smoke `run_project(12,cfg)`: PASS with `cfg.run.useTraceableDirs=true`, `cfg.run.runId='4cfa7eb'`, `cfg.core.monteCarlo=1`, `cfg.core.snapshots=200`, methods `ideal/oracle`, one target set for each source count, `4 deg` candidate strides, and `cfg.core.spice.maxIterations=20`.
- Result path: `results/4cfa7eb/`
...
- Change: added deterministic Case13 target/condition helpers, `benchmark_advantage_audit`, CSV outputs, delta RMSE heatmaps, win/loss summary, and oracle-gap plot.
- Change: made `auditSmoke` a practical subset (`calibrationCounts = [5 9]`, `SNR = [0 10]`, `strata = center/edge`, `monteCarlo = 3`) because the full matrix is bottlenecked by three-source `triplet_grid_ml`.
- Affected cases: new Case13 only. Case12 default remains unchanged.
- Validation: `matlab -batch "addpath(genpath(pwd)); run('tests/run_sanity_tests.m')"`; `checkcode default_config.m run_project.m src/benchmark_advantage_audit.m src/case13_helpers.m tests/run_sanity_tests.m`; traceable `run_project(13,cfg)` with `cfg.case13.profile='auditSmoke'`.
- Result path: `results/local-ee2e48be/`
- Case outputs: `case13_backend_switched_advantage_audit/`

#### Case13 smoke observations

- Summary rows: `40`
- V3.3 vs ARD count: win `2`, neutral `36`, loss `2`
- V3.3 vs best non-oracle baseline count: win `0`, neutral `21`, loss `19`
- Largest observed V3.3 loss vs best baseline: calibration `9`, SNR `0`, three-source edge hard, best baseline `interp`, V3.3 RMSE `5.0405`, best baseline RMSE `3.1117`, delta `1.9288`.
- The failure table is dominated by hard multi-source conditions, especially three-source edge/hard and some two-source hard cases.

#### Interpretation

- Smoke evidence supports the concern that V3.3 does not retain a broad frontend advantage after switching to source-count-aware backends.
- V3.3 is mostly neutral vs ARD in this smoke, but loses frequently against the best non-oracle calibrated baseline.
- This is smoke evidence only. It is enough to guide the next decision, but not a final `auditFull` benchmark.

#### Key images

![case13 v33 vs best baseline](assets/case13-v33-vs-best-baseline-local-ee2e48be.png)

![case13 v33 win rate](assets/case13-v33-win-rate-local-ee2e48be.png)

![case13 oracle gap](assets/case13-oracle-gap-local-ee2e48be.png)
