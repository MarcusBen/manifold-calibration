...
for sourceIdx = 1:numel(sourceValues)
    ...
    for calIdx = 1:numel(calValues)
        for snrIdx = 1:numel(snrValues)
            ...
            values(calIdx, snrIdx) = local_case13_mean_omit_nan(summaryTable.(fieldName)(mask));
        end
    end
...
for sourceIdx = 1:numel(sourceValues)
    ...
    gapValues(sourceIdx, 1) = local_case13_mean_omit_nan(summaryTable.deltaRmseV33MinusOracle(mask));
    gapValues(sourceIdx, 2) = local_case13_mean_omit_nan( ...
        summaryTable.bestBaselineRmse(mask) - summaryTable.oracleRmse(mask));
end
...
function value = local_case13_mean_omit_nan(values)
...
if isfield(cfg, 'case13')
    fprintf(fid, '## Case 13 Backend-Switched Advantage Audit\n\n');
    fprintf(fid, '- Profile: `%s`\n', cfg.case13.profile);
    fprintf(fid, '- Calibration counts: `%s`\n', mat2str(cfg.case13.calibrationCounts));
    fprintf(fid, '- SNR dB: `%s`\n', mat2str(cfg.case13.snrDb));
    fprintf(fid, '- Snapshots: `%d`\n', cfg.case13.snapshots);
    fprintf(fid, '- Smoke Monte Carlo: `%d`\n', cfg.case13.auditSmokeMonteCarlo);
    if isfield(cfg.case13, 'auditSmokeCalibrationCounts')
        fprintf(fid, '- Smoke calibration counts: `%s`\n', mat2str(cfg.case13.auditSmokeCalibrationCounts));
    end
    if isfield(cfg.case13, 'auditSmokeSNRDb')
